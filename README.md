...
没有配置 `OPENAI_API_KEY` 时，普通聊天会提示需要配置模型服务；商品相关请求仍会先走本地商品库覆盖性检查，匹配到商品时可用本地规则返回商品卡片。

前端使用 `/api/recommend/stream` 接收 SSE 流式输出，并会携带最近几轮连续对话上下文。后端每轮会先运行 Query Planner，把当前消息规划为 `chat`、`catalog_overview`、`search` 或 `recommend`：普通聊天才进入聊天模型；商品库概览由后端直接返回类目摘要；查询有无/价格走商品库搜索但不生成卡片；明确推荐才进入推荐流程。推荐流程会先用当前商品库做覆盖性检查：如果用户要的品类或商品与商品库没有明确匹配，会直接在主聊天窗口说明暂无该类商品并询问是否查看现有类目，不生成右侧商品卡片；只有通过覆盖性检查后，才会从候选商品中生成推荐卡片和二维码。
...
每一行是一条 JSON 记录。当前日志结构为 `schemaVersion: 2`，包含请求 ID、浏览器会话 ID、开始/结束时间、耗时、登录用户名（如已启用鉴权）、客户端信息、意图规划 `intentPlan`、推荐模式、工具调用、商品库覆盖性检查、推荐请求、推荐商品明细和错误信息等。连续对话内容保存在 `conversation` 字段：
