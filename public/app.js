...
function boot() {
  ...
  addMessage({
    role: "agent",
    text: "你好，我是店铺智能推荐助手。你可以先聊需求、预算或场景；需要我推荐具体商品时，我会生成商品卡片和扫码入口。",
    chips: starters
  });
  resizeInput();
}
...
function setRecommendationLoading() {
  ...
  recommendationList.innerHTML = `
    <section class="recommendation-empty loading-card">
      ...
      <strong>正在挑选商品</strong>
      <span>模型已决定生成推荐，商品卡片和二维码会在下方出现。</span>
    </section>
  `;
}
...
function rememberTurn(userText, assistantText) {
  ...
  if (cleanUserText) conversationHistory.push({ role: "user", content: cleanUserText });
  if (cleanAssistantText) conversationHistory.push({ role: "assistant", content: cleanAssistantText });
...
async function requestRecommendationStream(message, history, handlers) {
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    await requestRecommendationStream(message, historySnapshot, {
      meta(data) {
        if (data.mode === "selecting") {
          ...
          setRecommendationStatus("等待需求", "idle");
        }
      },
    ...
    stream.finish();
    rememberTurn(message, stream.getText());
  } catch (error) {
